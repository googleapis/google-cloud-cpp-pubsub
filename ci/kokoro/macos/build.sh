...
if [[ -n "${KOKORO_GFILE_DIR:-}" ]] &&
   [[ -f "${KOKORO_GFILE_DIR}/pubsub-integration-tests-config.sh" ]]; then
  source "${KOKORO_GFILE_DIR}/pubsub-integration-tests-config.sh"
  GOOGLE_APPLICATION_CREDENTIALS="${KOKORO_GFILE_DIR}/pubsub-credentials.json"
